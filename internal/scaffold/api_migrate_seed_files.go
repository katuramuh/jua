...
func Seed(db *gorm.DB) error {
	...
	// Seed Jua UI component registry (original 4 + 87 extended)
	...
	blogs := []models.Blog{
		{
			...
			Excerpt:     "Learn how to build full-stack applications with Jua, the Go + React meta-framework.",
			Content:     ` + "`" + `<h2>What is Jua?</h2><p>Jua is a full-stack meta-framework that combines <strong>Go</strong> (Gin + GORM) on the backend with <strong>Next.js</strong> (React + TypeScript) on the frontend. It gives you everything you need to build modern web applications — authentication, admin panel, file uploads, background jobs, and more.</p><h2>Quick Start</h2><p>Getting started is as simple as running a single command:</p><pre><code>jua new my-project</code></pre><p>This scaffolds a complete monorepo with:</p><ul><li>A Go API with JWT auth, RBAC, and CRUD endpoints</li><li>A Next.js web app with login, register, and dashboard</li><li>An admin panel with data tables, forms, and widgets</li><li>Docker setup for PostgreSQL, Redis, and MinIO</li></ul><h2>Why Jua?</h2><p>Most full-stack frameworks force you into a single language. Jua lets you use the <em>best tool for each job</em> — Go for performance-critical backend logic, React for rich interactive UIs, and shared TypeScript types to keep everything in sync.</p><blockquote><p>Build with confidence. Ship with Jua.</p></blockquote>` + "`" + `,
			Image:       "",
			Published:   true,
			PublishedAt: &past1,
		},
